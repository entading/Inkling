...
1. 四处 `package.json` 同步 bump 版本（根 / `server/` / `web/` / `desktop/`——桌面版 version 即安装包与「关于」卡展示的版本）并提交（日常开发在 develop）
...
4. **发布基线是 master**：先推送 develop 并快进 master（`git push origin develop` && `git push origin develop:master`），再 `gh release create vX.Y.Z --target master --notes "…"`（`--target` 解析的是远端分支，先推才能让 tag 指向正确提交），上传资产三件：`Inkling-Setup-X.Y.Z.exe`、`latest.yml`、`Inkling-Setup-X.Y.Z.exe.blockmap`（**资产名须与 latest.yml 内 url 一致（连字符形式），缺 latest.yml 应用内更新通道即失效**）
