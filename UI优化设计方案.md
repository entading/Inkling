...
| **M5' 编辑器·列表·移动端** ✅ 已完成（2026-09-01，代码 `570a213`，记录 `UI-M5实施记录.md`） | §9、§10 | 编辑器增强不破坏撤销与全量源码语义（execCommand('undo') 8 步逐操作回退实证；Ctrl+S 无改动零请求/改动 saved+PUT；Tab 两空格；工具条包裹·占位·行首插入；`[[` 补全跨板块候选 cap8 + vocab 裸 slug/非 vocab 前缀 + Esc/Tab/blur 关闭；分屏滚动 50%↔50% 单向映射）；板块筛选（chips 聚合数量一致、单选/多选 OR、与搜索 AND、URL 直达与 back 回溯、sort 切换顺序断言、密度两档+localStorage、stagger/skeleton 零破坏）；FAB（375 出现点击→/new、/new 隐藏）与回顶（>600px 桌面+移动、reduced-motion auto 降级）；12 页 × 浅深走查 24 项零横滚；preview 4173 生产构建复核；另见记录「IAB 遮挡节流两次发作」的验证降级口径 |

每里程碑通用验收：`npm run build`（tsc + vue-tsc + vite）通过；Playwright 实测记录；报告落根目录；遵守 AGENTS.md 全部约束。

### UI 焕新阶段收官（2026-09-01）

五个里程碑全部完成交付，A 视觉层（A1–A5）与 B 交互层（B1–B5）全量落地，阶段目标「视觉档次、阅读体验、操作效率」三条线收官：

| 里程碑 | 代码 | 记录 |
|---|---|---|
| M1' 令牌收敛与深色主题 | `efccdeb` | `UI-M1实施记录.md` |
| M2' 排版·图标·板块色 | `91fddfb` | `UI-M2实施记录.md` |
| M3' 微动效 | `36da2da` | `UI-M3实施记录.md` |
| M4' 命令面板与阅读增强 | `b476fa2`（复检修复 `67fa364`） | `UI-M4实施记录.md` |
| M5' 编辑器·板块筛选·移动端 | `570a213` | `UI-M5实施记录.md` |

运行时依赖如定稿仅此一项新增：`@vueuse/core`（M1' 引入，theme.ts 使用），M2'–M5' 零再增；C 进阶方向（知识图谱 / PWA / 学习辅助）按定稿范围本轮不做，留待后续阶段再议。各里程碑约束与坑位已沉淀至 `AGENTS.md`「UI 焕新阶段补充约定」。
