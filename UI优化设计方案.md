...
| **M2' 排版·图标·个性色** | §3、§4、§5 | 图标全站替换无缺失；tag/板块色深浅两态可读性；衬线正文浅/深双主题通读检查笔画；字体分片按需加载验证（Network 面板：首屏仅命中分片、无全量下载、swap 无明显闪替） |
| **M3' 微动效** | §6 | 骨架屏三处；转场/stagger 顺滑；reduced-motion 下全静态 |
| **M4' 命令面板与阅读增强** | §7、§8 | Ctrl+K 全键盘可用；TOC/进度/上下篇/快捷键/复制全通过 |
